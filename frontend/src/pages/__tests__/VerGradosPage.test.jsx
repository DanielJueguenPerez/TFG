...
import VerGradosPage from "../VerGradosPage";
...
vi.mock("../../components/ListaPaginada", () => {
  ...
  return {
    ...
    default: (props) => {
      ...
      const items = [
        { id_grado: 1, nombre: "Grado de ejemplo 1" },
        { id_grado: 2, nombre: "Grado de ejemplo 2" },
      ];
...
describe("VerGradosPage", () => {
  ...
  it("Se pasan las props correctas a ListaPaginada", () => {
    ...
    expect(listaPaginadaProps.recuperarDatos).toBe(verGrados);
  ...
  it("Se muestra el título y el botón de volver atrás", () => {
    ...
    expect(screen.getByRole("heading", { name: /grados disponibles/i })).toBeInTheDocument();
  ...
  it("Se renderiza la lista usando renderItem", () => {
    ...
    const link1 = screen.getByText("Grado de ejemplo 1");
    expect(link1).toHaveAttribute("href", "/grados/1");

    const link2 = screen.getByText("Grado de ejemplo 2");
    expect(link2).toHaveAttribute("href", "/grados/2");
  });
});
